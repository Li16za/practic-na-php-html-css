<?php 
	...
	$ID = $_POST['ID'];
	if($ID != '') {
	    $qry = " SELECT * FROM A_criminal_record WHERE Id_a_criminal_record =  " . $ID ; //запрос к MySQL
	}	       
	$result = $conn->query($qry);
	if ($result->num_rows > 0) {           
		$sql = "DELETE FROM A_criminal_record WHERE Id_a_criminal_record = " . $ID ;
