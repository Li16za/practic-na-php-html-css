<?php 
	...
	$ID = $_POST['ID'];
	if($ID != '') {
	    $qry = " SELECT * FROM Criminal_case WHERE Id_criminal_case =  " . $ID ; //запрос к MySQL
	}	       
	$result = $conn->query($qry);
	if ($result->num_rows > 0) {           
		$sql = "DELETE FROM Criminal_case WHERE Id_criminal_case = " . $ID ;
